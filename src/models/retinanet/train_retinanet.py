...
CHECKPOINT_DIR = PROJECT_DIR / "checkpoints_retinanet"
MODEL_OUTPUT_PATH = PROJECT_DIR / "retinanet_tulip.pth"
...
def latest_checkpoint(checkpoint_dir: Path):
    checkpoints = sorted(checkpoint_dir.glob("retinanet_epoch_*.pth"))
...
def main():
    parser = argparse.ArgumentParser(description="Train RetinaNet on the Roboflow tulip dataset.")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (images, targets) in enumerate(train_loader):
            ...
            for target in targets:
                fixed_target = {key: value.to(device, non_blocking=True) for key, value in target.items()}
        ...
        stabilize_cuda()
        checkpoint_path = CHECKPOINT_DIR / f"retinanet_epoch_{epoch + 1}.pth"
        torch.save(
            {
                "epoch": epoch + 1,
